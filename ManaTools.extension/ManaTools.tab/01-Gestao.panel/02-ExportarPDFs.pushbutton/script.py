...
if not all_sheets:
    forms.alert("Não há pranchas.", exitscript=True)

sheet_options = [SheetListItem(s) for s in all_sheets]
selected_wrappers = forms.SelectFromList.show(
    sheet_options,
    multiselect=True,
...
for sheet in selected_sheets:
    ...
    if not ENFORCE_REVISION or has_rev:
        valid_sheets.append(sheet)

if not valid_sheets:
    forms.alert("Nenhuma prancha válida.", exitscript=True)

# --- 3. Configuração ---
export_mode = forms.CommandSwitchWindow.show(
    ["Arquivos Separados", "Arquivo Único"], message="Formato:"
)
if not export_mode:
    script.exit()
    ...
    if export_mode == "Arquivo Único":
        try:
            ...
            name = forms.ask_for_string(
                default=safe_title, prompt="Nome:", title="PDF Combinado"
            )
            if name:
        ...
        for i, sheet in enumerate(valid_sheets):
            ...
            try:
                ...
                if c_num in ["ERRO_NUM", "Folha"]:
                    c_num = "ID_" + str(sheet.Id)
        ...
        if count_success > 0:
            forms.toast(
                "{} arquivos gerados.".format(count_success), click=dest_folder
            )
